...
def find_isomorphisms(G1, G2):
    """
    Nájde všetky topologické izomorfizmy medzi dvoma grafmi (zanedbáva váhy).
    
    :param G1: Prvý graf (Graph objekt)
    :param G2: Druhý graf (Graph objekt)
    :return: (bool, list of mappings)
    """
    if len(G1.vertices) != len(G2.vertices):
        return False, []  # Musia mať rovnaký počet vrcholov

    # Všetky možné priradenia vrcholov G1 k vrcholom G2
    ...
    for perm in itertools.permutations(vertex_names_G2):
        ...
        # Overíme, či toto mapovanie zachováva topologickú štruktúru
        valid = True
        for v1 in G1.vertices.values():
            ...
            for neighbor, _ in v1.neighbors:
                ...
                # Skontrolujeme, či sa v G2 nachádza tá istá hrana
...
print("Sú grafy izomorfné?", find_isomorphisms(G1, G2))  # Očakáva sa True
